		...
		if("success".equals(msg)){
			...
			try {
				...
				if(type == 1){
					...
					Page<DepartmentManager> page = new Page<DepartmentManager>(1, 10);
					page.getSearchProperties().add(new SearchProperty("name", name, Operator.EQ));
					...
					if(page.getConten().size() == 0){
						...
						DepartmentManager departmentManager = page.getConten().get(0);
						if(!password.equals(departmentManager.getPassword())){
							msg = "密码错误！";
						}else{
							req.getSession().setAttribute("user", departmentManager);
							req.getSession().setAttribute("userType", type);
						}
					}
				}else{
